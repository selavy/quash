...
void interactive(char **envp) {
  for(;;) {
    ...
    if (0==strcmp ("exit",command)) {
      ...
      execute_command (command, last_token, envp);
    } else {
      int c;
...
void execute_command(char * command, int last_token, char **envp) {
  int status;
  pid_t pid;
  int i;
  ...
  if (FORK_ERROR==pid) {
    ...
    if (last_token) {
      execle (command, command, (char *) NULL, envp);
      } else  {
      execve (command, args_list, envp);
    }
  } else {
    /* parent */
